...
export default function Account() {
  ...
  const oauth = useMutation({ mutationFn: async (provider: Provider) => {
    if (!supabase) throw new Error('Accounts are not configured. Please try again later.');
    const { error } = await supabase.auth.signInWithOAuth({ provider, options: { redirectTo: `${window.location.origin}/account` } });
    if (error) throw new Error(provider === 'google' ? 'Google sign-in is not enabled on this project yet.' : provider === 'apple' ? 'Apple sign-in is not enabled on this project yet.' : 'Social sign-in failed.');
  } });
